 ...
 */
function detectCollectionContext(currentFilePath, workspaceRoot) {
...
function activate(context) {
    let disposable = vscode.commands.registerCommand('ember-hopper.search', async () => {
        try {
            ...
            const useEmojis = config.get('showEmojis', true);
            // 1. Detect Base Entity (Handles Pods vs Classic)
            let fileName = path.basename(currentFilePath, path.extname(currentFilePath));
            const genericNames = [
                'component',
                'template',
                'route',
            ...
            const collectionContext = detectCollectionContext(currentFilePath, workspaceRoot);
            let files = [];
            await vscode.window.withProgress({
                ...
                if (collectionContext.type !== 'other') {
                    ...
                    searchDirs.push({ dir: currentFileDir, depth: 4 }, // Start with current directory
                    ...
                    const dirCheckResults = await Promise.all(allDirsToCheck.map(async (dirName) => {
                ...
                const searchPromises = Array.from(uniqueDirs.entries()).map(async ([searchDir, maxDepth]) => {
                    try {
                        return await findFilesInDirectory(searchDir, baseEntity, maxDepth);
                    }
                    catch (error) {
                        return [];
                    }
                ...
                files = files.filter((file) => file.fsPath !== currentFilePath);
            });
            if (files.length === 0) {
                vscode.window.showInformationMessage(`No related files found for "${baseEntity}"`);
                return;
            }
...
function getEmberLabel(filePath, useEmojis) {
...
async function findFilesInDirectory(dir, baseEntity, maxDepth, currentDepth = 0, visited = new Set()) {
    const files = [];
        ...
        for (const entry of entries) {
            ...
            if (entry.isDirectory()) {
                ...
                if (currentDepth < maxDepth &&
                    ...
                    subdirPromises.push(findFilesInDirectory(fullPath, baseEntity, maxDepth, currentDepth + 1, visited));
                }
            }
                ...
                else if ((fileName === `${baseEntity}-test` ||
